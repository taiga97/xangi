...
2. **Idle session reset**: For non-reset messages, if the current session's `updatedAt` is older than `LINE_IDLE_RESET_HOURS` (default 12 h), `hasSessionGoneIdle()` returns true → `archiveSession()` is called, then `ensureSession()` creates a new session. Conversations separated by half a day or more are treated as separate sessions. The `logs/sessions/*.jsonl` files are kept after archiving, so history is preserved.
